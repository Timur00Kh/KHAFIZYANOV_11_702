<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
        ...
        <ul class="navbar-nav dropdown">
            <li class="nav-item">
                <input id="nav_search" class="form-control mr-sm-2 dropdown-toggle" type="search" placeholder="Search" aria-label="Search">
                <div class="dropdown-menu mt-2" id="nav_search_container" style="width: 100%; " aria-labelledby="searchDropdownToggle">
                    <a class="dropdown-item py-0 px-3 nav_search_el" href="#">
                        <div class="row ">
                            <div class="col-9 search-name" style="overflow: hidden; text-overflow: ellipsis;">name</div>
                            <div class="col-1 search-year"><span class="small">2018</span></div>
                        </div>
                    </a>
                </div>
            </li>
        ...
        <ul class="navbar-nav ml-2" id="user_min" style="display: ${user.isPresent()?then("", "none")}">
            <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle dropdown-toggle-split" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <img id="user_min_img" class="rounded-circle img-fluid" style="height: 40px; border-width: 1px !important;" src='${(user.isPresent() && user.get().getAvatarUrl()??) ? then(user.get().getAvatarUrl(), "/img/default_avatar.png")}'>
                </a>
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="userDropdown">
                    <a class="dropdown-item" id="user_min_name" href="/profile">${user.isPresent()?then(user.get().getName(),'')}</a>
        ...
        <ul class="navbar-nav ml-2" id="login_btn" style="display: ${user.isPresent()?then("none", "")}">
            <li class="nav-item">
...
<script src="http://localhost/js/nav_search.js"></script>
